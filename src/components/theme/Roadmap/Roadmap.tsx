  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap
        ...
        .fromTo(
          ".gsap-vertical-line-up",
          { scaleY: 0, transformOrigin: "bottom" },
          { scaleY: 1, duration: 0.7, ease: "power2.out" }
        )
        .fromTo(
          ".gsap-vertical-line-down",
        ...
        <div className={cx("wrapper-inner")}>
          ...
          <Slider {...settings}>
            {timelines
              .concat(timelines)
              .map(function (
                { title, description, datetime }: RoadmapType,
                index: number
              ) {
                      ...
                      >
                        ...
                        <div
                          className={
                            ...
                            (!isEvent
                              ? " gsap-timeline-content-odd"
                              : " gsap-timeline-content-even")
                          }
                        >
                          <h3 className={cx("timeline-title")}>{title}</h3>
                          <p className={cx("timeline-description")}>
                            {description}
                          </p>
                        </div>
                      </div>
